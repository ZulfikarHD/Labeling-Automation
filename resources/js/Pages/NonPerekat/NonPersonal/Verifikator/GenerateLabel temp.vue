                    ...
                    <div class="flex justify-between gap-6 mx-auto w-full">
                        ...
                        <div class="flex-grow">
                            ...
                            <template v-if="form.no_rim !== 999">
                                <TextInput id="no_rim" ref="no_rim" v-model="form.no_rim" type="number"
                                    ...
                                    autocomplete="no_rim" min="1" @input="updateValues" />
                            </template>
                            <template v-else>
                        ...
                        <div class="flex-grow">
                            ...
                            <TextInput id="lbr_ptg" ref="lbr_ptg" v-model="form.lbr_ptg" type="text"
                                class="block bg-slate-200/80  drop-shadow shadow w-full px-4 py-2 mt-2 text-lg text-center font-bold"
                                autocomplete="lbr_ptg" @input="updateValues" disabled />
                        </div>
                    </div>
...
// Fungsi untuk memperbarui nilai form.no_rim dan form.lbr_ptg secara real-time
const updateValues = () => {
    // Logic to update form.no_rim and form.lbr_ptg in real-time
    // For example, you can log the values or perform any other action
    console.log('Rim Number:', form.no_rim);
    console.log('Cutting Sheet:', form.lbr_ptg);
};
...
const fetchUpdatedValues = async () => {
    try {
        ...
        form.no_rim = response.data.no_rim; // Update form.no_rim with the fetched data
        form.lbr_ptg = response.data.lbr_ptg; // Update form.lbr_ptg with the fetched data
    } catch (error) {
...
onMounted(() => {
    intervalId = setInterval(fetchUpdatedValues, 500); // Fetch every 0.5 seconds
});
