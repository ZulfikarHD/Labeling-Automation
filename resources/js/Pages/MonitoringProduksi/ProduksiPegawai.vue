...
<template>
    ...
    <AuthenticatedLayout>
        <div class="px-4 py-6 mx-auto w-full">
            <div class="px-10 mx-4 mb-4 ml-28 justify-center flex flex-col items-center">
                <InputLabel for="dateFilter" class="pb-2 pl-1 text-lg font-medium text-gray-700">Tanggal Produksi</InputLabel>
                <TextInput id="dateFilter" type="date" v-model="form.date" class="border border-gray-300 rounded-md shadow-sm focus:ring focus:ring-blue-300 max-w-2xl"/>
            </div>
            <div class="w-full flex flex-wrap justify-center items-baseline gap-4 mb-10">
                <!-- Team Verifikasi-->
                <template v-for="team in props.teams" :key="'team'+team.id">
                    <div class="flex flex-col justify-center gap-2 rounded-lg p-4">
